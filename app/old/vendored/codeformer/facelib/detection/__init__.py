...
# Absolute path to models/facelib/ at the project root (6 levels up from this file).
_FACELIB_MODEL_DIR = os.path.normpath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), *(['..'] * 6), 'models', 'facelib')
)
...
def init_retinaface_model(model_name, half=False, device="cuda"):
    ...
    model_path = load_file_from_url(url=model_url, model_dir=_FACELIB_MODEL_DIR, progress=True, file_name=None)
